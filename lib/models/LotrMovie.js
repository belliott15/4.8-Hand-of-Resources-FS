...
module.exports = class LotrMovie{
  ...
  static async insert({ name, release, extended_edition }){
    const { rows } = await pool.query('INSERT INTO lotrmovies (name, release, extended_edition) VALUES ($1, $2, $3) RETURNING *', 
      [name, release, extended_edition]);
    return new LotrMovie(rows[0]);
  }
