const lotrMovies = require('../controllers/lotrMovies');
...
module.exports = class LotrMovie{
  ...
  static async update(id, attrs){
    const updatedMovie = await new LotrMovie.getById(id);
    if(!updatedMovie) return null;
    const { name, released, extended_edition } = { ...updatedMovie, ...attrs };
    const { rows } = await pool.query('UPDATE lotrmovies SET name=$2, released=$3, extended_edition=$4 WHERE id=$1 RETURINING *', 
